...
if [[ $ACTIVE != *prod* ]]; then
    # 物理网卡
    PYHSICAL_NETWORK=$(ls /sys/class/net/ | grep -v "$(ls /sys/devices/virtual/net/)")
    # 获取物理网卡的 ip，JMX 配置 IP
    JMX_HOSTNAME=$(${IFCONFIGEXE} ${PYHSICAL_NETWORK}|grep inet|grep -v 127.0.0.1|grep -v inet6|awk '{print $2}'|tr -d "addr:")
else
    # jmx 本机ip
...
if [[ $ACTIVE != *prod* ]]; then
  ...
  ## JMX Options
  JMX_OPTS="-Djava.rmi.server.hostname=${JMX_HOSTNAME} -Dcom.sun.management.jmxremote.port=4${PORT:1} -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false"
  ## Remote debug
